...
class TestBart(unittest.TestCase):
    ...
    @classmethod
    def setUpClass(cls):
        cls.model = torch.hub.load('pytorch/fairseq', 'bart.large.cnn').eval()
    ...
    def test_cnn_generation(self):
        ...
        bsz = 1
        count = 1
        hypotheses_batch = bart.sample([FRANCE_ARTICLE, SHORTER_ARTICLE], beam=4,
                                       lenpen=2.0, max_len_b=140,
                                       min_len=55, no_repeat_ngram_size=3)

        print(hypotheses_batch)
        self.assertEqual(hypotheses_batch, [EXPECTED_SUMMARY_FRANCE,DESIRED_RESLT_SHORTER])

    def test_cnn_iran_case(self):
        bart = self.model
        hypotheses_batch = bart.sample([IRAN_ARTICLE], beam=4,
                                       lenpen=2.0, max_len_b=140,
                                       min_len=55, no_repeat_ngram_size=3)

        print(hypotheses_batch)

    def test_cnn_subway_case(self):
        bart = self.model
        hypotheses_batch = bart.sample([SUBWAY_ARTICLE], beam=4,
                                       lenpen=2.0, max_len_b=140,
                                       min_len=55, no_repeat_ngram_size=3)
    ...
    def test_arizona_subway_case(self):
        self.run_example(ARIZONA_ARTICLE)

    def test_cnn_barca(self):
        self.run_example(BARCA_ARTICLE)

    def test_cnn_id(self):
        self.run_example(self.lns[2568:2576])
